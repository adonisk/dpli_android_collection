...
public class CollectionsPayCardApiHelper extends ApiHelperBase implements ApiHelper {
	...
	public void callApi(JSONObject o, Activity callingActivity, int requestCode) {
		...
		try {
			...
			intent.putExtra(EzeConstants.KEY_USERNAME, ctx.getUserName());
			...
			ad.put("_status", "CARD_PAYMENT");
			ad.put("_status_id", "CARD_PAYMENT");
			...
			if(amount <= 0) {
				UIUtils.showToast("incorrect_amount", callingActivity);
				return;
			}
